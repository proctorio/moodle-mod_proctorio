...
$string['quiz_configurations_help'] = "<strong>⚠️ Write SQL Queries for Custom Quiz Types</strong><br><br><strong>Format (one line per quiz type):</strong><br><code>modname|SELECT state AS attempt_status, attempt AS attempt_number FROM {tablename} WHERE userid = :userid AND quiz = :quizid ORDER BY timemodified DESC</code><br><br><strong>Requirements:</strong><br>• Must use <code>:userid</code> and <code>:quizid</code> placeholders in WHERE clause<br>• Must have aliases: <code>AS attempt_status</code> and <code>AS attempt_number</code><br>• <strong>You CAN reuse the same placeholders multiple times</strong> (e.g., in subqueries - they're automatically handled)<br>• Must include ORDER BY to get the latest attempt - see below, you do not need (and should not add) your own LIMIT<br>• Use Moodle table syntax: <code>{tablename}</code> (curly braces, no mdl_ prefix)<br><br><strong>Security restrictions (enforced automatically):</strong><br>• Must be a single <code>SELECT</code> statement - no semicolons, no stacked statements<br>• No write/DDL/admin keywords anywhere in the query (INSERT, UPDATE, DELETE, DROP, ALTER, CREATE, TRUNCATE, GRANT, REVOKE, MERGE, REPLACE, EXEC/EXECUTE, CALL, OUTFILE, DUMPFILE, LOAD_FILE)<br>• Comments (<code>--</code>, <code>#</code>, <code>/* */</code>) are stripped before the query runs, so the required placeholders/aliases above must appear in the real query, not just in a comment<br>• A line that fails these checks is rejected and logged (with debugging on); the plugin falls back to auto-detection for that module<br>• <strong>The plugin always fetches at most one row itself</strong> - any trailing <code>LIMIT</code>/<code>OFFSET</code> you write is stripped and ignored, so a query matching multiple rows can never return more than one. <code>ORDER BY</code> is still up to you and still matters: it decides which of the matching rows is \"the\" one returned.<br>• <strong>This still trusts whoever can edit this setting</strong>: a SELECT can read any table/column this database user can access. Only grant Moodle's site:config capability to admins you trust with that.<br><br><strong>Examples:</strong><br><code># Custom quiz module<br>quiz|SELECT state AS attempt_status, attempt AS attempt_number FROM {quiz_attempts} WHERE userid = :userid AND quiz = :quizid ORDER BY timemodified DESC<br><br># Adaptive quiz with subquery (notice :userid and :quizid used twice - this works!)<br>adaptivequiz|SELECT attemptstate AS attempt_status, (SELECT COUNT(*) FROM {adaptivequiz_attempt} WHERE userid = :userid AND instance = :quizid) AS attempt_number FROM {adaptivequiz_attempt} WHERE userid = :userid AND instance = :quizid ORDER BY timemodified DESC</code><br><br><strong>How to build your query:</strong><br>1. Find your table: <code>SHOW TABLES LIKE '%attempt%';</code><br>2. See columns: <code>DESCRIBE mdl_yourquiz_attempts;</code><br>3. Test query manually (add your own LIMIT here just to keep the console output short - this LIMIT is not part of what you paste into the setting): <code>SELECT * FROM mdl_yourquiz_attempts WHERE userid = 123 ORDER BY timemodified DESC LIMIT 1;</code><br>4. Add aliases and placeholders<br><br><strong>Notes:</strong><br>• Standard 'quiz' and 'adaptivequiz' work automatically (no config needed)<br>• One line per quiz type, use pipe (|) to separate modname from SQL<br>• Parameters :userid and :quizid can be used multiple times - they're automatically numbered internally<br>• Lines starting with # are comments<br>• Use subqueries if you need to count attempts";
